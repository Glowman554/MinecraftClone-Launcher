...
class Ui_Form(object):
    def setupUi(self, Form):
        ...
        self.download_progress = QProgressBar(Form)
        self.download_progress.setObjectName(u"download_progress")
        self.download_progress.setValue(0)
        self.download_progress.setAlignment(Qt.AlignCenter)
        self.download_progress.setTextVisible(True)
        self.download_progress.setOrientation(Qt.Horizontal)
        self.download_progress.setInvertedAppearance(False)
        self.download_progress.setTextDirection(QProgressBar.BottomToTop)

        self.gridLayout_2.addWidget(self.download_progress, 1, 0, 1, 3)
    ...
    def launch_game(self, Form):
        if(download_version() != get_loacl_version()):
            print("UPDATE")
            download_game(self.report_hook)
            update_done()
    ...
    def report_hook(self, count, blockSize, totalSize):
        self.percent = (count * blockSize) * 1e2 / totalSize
        self.download_progress.setValue(self.percent)
        qApp.processEvents()
